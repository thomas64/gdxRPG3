...
class SpellLearner private constructor(
    ...
    private val requiredSkill: Int = selectedHero.getSkillById(spellToLearn.skill).rank
    ...
    private val isOfSkill: Boolean = requiredSkill != -1
    private val hasRequiredSkill: Boolean = requiredSkill >= 1
    private val hasEnoughRequiredSkill: Boolean = requiredSkill >= spellToLearn.minSkill
    ...
    private fun learn() {
        val requiredSkillName: String = spellToLearn.skill.title
        when {
            selectedHero.isDead -> showError("${selectedHero.name} is deceased.")
            hasSpell -> showError("You already know $spellName.")
            !isOfSkill -> showError("${selectedHero.name} is not capable of learning $spellName.")
            !hasRequiredSkill -> showError("You need the $requiredSkillName skill to learn $spellName.")
            !hasEnoughRequiredSkill -> showError("Your $requiredSkillName skill is not high enough to learn $spellName.")
    ...
    private fun showConfirmDialog() {
        val question = """
            Are you sure you wish to learn $spellName
            to ${selectedHero.name} for $xpCost XP and $goldCost gold?""".trimIndent()
