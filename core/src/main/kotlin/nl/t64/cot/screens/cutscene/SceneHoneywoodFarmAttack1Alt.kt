...
class SceneHoneywoodFarmAttack1Alt : CutsceneScreen() {
    ...
    override fun prepare() {
        ...
        actions = listOf(luanaIsAlreadyRecruited(),
                         orcsWalkIn())
    }

    private fun luanaIsAlreadyRecruited(): Action {
    ...
    private fun orcsWalkIn(): Action {
        return Actions.sequence(
            ...
            Actions.addAction(Actions.sequence(
                ...
                Actions.run { orc1.entityState = EntityState.RUNNING },
                Actions.parallel(
                    actionMoveBy(orc1, -192f, -72f, 1.5f, FAST_STEP),
                    Actions.sequence(
                        Actions.delay(1f),
    ...
    override fun exitScreen() {
        endCutsceneAnd { BattleScreen.load("farm_battle1", this) }
    }

    override fun onNotifyBattleWon(battleId: String, spoils: Loot) {
        screenManager.setScreen(ScreenType.SCENE_HONEYWOOD_FARM_ATTACK_2_ALT)
        SpoilsCutsceneScreen.load(spoils, ScreenType.SCENE_HONEYWOOD_FARM_ATTACK_2_ALT)
    }
