...
data class InventoryItem(
    ...
    fun getSellValuePiece(totalMerchant: Int): Int {
        ...
        val durabilityValue: Float = if (maxDurability > 0 && durability < maxDurability) {
            price * (durability.toFloat() / maxDurability.toFloat()).coerceAtMost(1f)
        } else {
            price.toFloat()
        }
        val sellValue: Float = floor(durabilityValue / 3f)
        return (sellValue + ((sellValue / 300f) * totalMerchant)).roundToInt().coerceAtLeast(1)
    }
